...
void rand_quest(int numOfQuest, NODE_QUEST *chosenQuest);
void updatePoint(NODE_USER *user);
void play_showpoint(int sockfd, int mode);
...
NODE_USER *registFunc(NODE_USER *head_User, int connfd)
{
	...
	if (checkindex != 0)
	{
		...
		int err_msg = 1;
		int n = send(connfd, &err_msg, sizeof(int), 0);
		if (n < 0)
			perror("Error: ");
	...
	fclose(fptr);
	int msg = 0;
	int n = send(connfd, &msg, sizeof(int), 0);
	if (n < 0)
		perror("Error: ");
	return head_User;
}
...
NODE_USER *loginFunc(NODE_USER *head_User, int connfd)
{
	...
	while (NULL != checkNode)
	{
		...
		if (checkindex == 0)
		{
			...
			{ // dung pass
				int message = 0;
				int n = send(connfd, &message, sizeof(int), 0);
				if (n < 0)
					perror("Error: ");
				return checkNode;
			}
...
void play_showpoint(int sockfd,int mode)
{
	...
	case 1:
		...
		int user_ans;
		if (recv(sockfd, &user_ans, sizeof(int), 0) <= 0)
			perror("Error: ");
		else
		{
			if (user_ans == chosenQuest->true_ans)
			{
				//updatePoint(client_info);
				printf("True Ans\n");
				int correct_msg = 1;
				send(sockfd, &correct_msg, sizeof(int), 0);
			}
			else{
				printf("Incorrect answer\n");
				int incorrect_msg = 0;
				send(sockfd,&incorrect_msg,sizeof(int),0);
			}
		}
		break;
	case 2:
